-- Cotas inclusas por aluno/ano: 2 relatórios de desenvolvimento, 2 portfólios texto, 1 imagem portfólio.

alter type public.ai_entitlement_type add value if not exists 'portfolio_text';
...
begin
  ...
  if p_entitlement_type is not null
    ...
    and greatest(0, coalesce(p_semester_included_quantity, 0)) > 0
    and p_entitlement_type in (
      'development_report'::public.ai_entitlement_type,
      'portfolio_text'::public.ai_entitlement_type,
      'portfolio_image'::public.ai_entitlement_type
    ) then
    ...
    if v_entitlement.id is not null then
      ...
      if v_entitlement.used_quantity < v_entitlement.included_quantity then
        ...
        select
          true,
          null::text,
          'Uso registrado pela cota inclusa.'::text,

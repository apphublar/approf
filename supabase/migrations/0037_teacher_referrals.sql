create table if not exists public.teacher_referrals (
  ...
  status text not null default 'registered'
    check (status in ('registered', 'converted', 'rewarded', 'invalid')),
  credit_cents integer not null default 0,
  giztokens_bonus integer not null default 0,
...
create index if not exists referral_credits_owner_idx
  on public.referral_credits(owner_id, expires_at desc);

alter table public.teacher_referrals enable row level security;
alter table public.referral_credits enable row level security;

drop policy if exists "teacher_referrals_participant_read" on public.teacher_referrals;
create policy "teacher_referrals_participant_read" on public.teacher_referrals
for select using (
...
drop policy if exists "referral_credits_owner_read" on public.referral_credits;
create policy "referral_credits_owner_read" on public.referral_credits
...
begin
  ...
  if lower(v_subscription.plan) not in ('monthly', 'mensal', 'annual', 'anual') then

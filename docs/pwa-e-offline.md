...
2. Salvar anotações offline com status `pending_sync`.
...
6. Evitar cachear fotos de crianças sem necessidade.

## Regra de privacidade offline

Dados sensíveis em cache local devem ser minimizados. Fotos de crianças não devem ser salvas agressivamente no cache do service worker.
